...
```text
public/features/divine-throne/thrones/{assetId}.png
```
...
Der Kopier- und Mapping-Schritt ist im Android-Arbeitsbereich dokumentiert/automatisiert:

```text
/Users/daschultheiss/android/apply_throne_assets_to_hero_database.py
```

Das Script macht im Kern:

- `*.sprite.png` unter `throne_assets_all` suchen
- nur `new_texture_ui_throneequip_throneframe_a_ui_throneequip_frame_*` verwenden
- Asset-ID aus dem Bundle-Namen ableiten
- PNG nach `public/features/divine-throne/thrones/{assetId}.png` kopieren
- `src/data/divine-throne.json` mit `throneAssetId` aktualisieren
- fehlende Hero-zu-Asset-Mappings hart fehlschlagen lassen

## Ergebnis

Im Repo liegen jetzt normale statische Web-PNGs:
